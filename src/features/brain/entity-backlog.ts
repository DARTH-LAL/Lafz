...
type EnqueueEntityBacklogBatchOptions = {
  limit?: number;
};

type EntityBacklogBatchResult = {
...
export function getEntityBacklogRefillCooldownMs() {
...
export function isEntityBacklogAutoRefillEnabled() {
...
function buildEntityAgentJobKey(spotifyTrackId: string, generatedAt: string) {
  return ["entity_agent", ENTITY_AGENT_JOB_VERSION, spotifyTrackId, generatedAt].join("::");
}

function buildEntityPayload(draft: UnknownRecord, songNodeId: string, fallbackTrackId: string, fallbackGeneratedAt: string) {
...
export async function hasActiveEntityAgentJobs() {
  ...
  if (error) {
    console.error("[lafz-brain] could not count active entity jobs.", error);
    return false;
  }
...
export async function enqueueEntityBacklogBatch(
  options: EnqueueEntityBacklogBatchOptions = {}
): Promise<EntityBacklogBatchResult> {
  const supabase = getSupabaseServerClient();
  ...
  const [draftRows, worldRows, existingJobRows] = await Promise.all([
    ...
    console.error("[lafz-brain] could not scan entity backlog.", error);
    return [[], [], []] as const;
  });
  ...
  for (const row of draftRows) {
    ...
    const jobKey = buildEntityAgentJobKey(spotifyTrackId, generatedAt);
    ...
    candidates.push({
      job_key: jobKey,
      job_type: "entity_agent",
      status: "pending",
      scope_type: "song",
      scope_key: spotifyTrackId,
      spotify_track_id: spotifyTrackId,
      priority: 165,
      payload_json: buildEntityPayload(draft, songNodeId, spotifyTrackId, generatedAt)
    });
  ...
  const { data, error } = await supabase.from("agent_jobs").upsert(rows, { onConflict: "job_key" }).select("job_key");

  if (error) {
    console.error("[lafz-brain] could not enqueue entity backlog jobs.", error);
    return {
      candidatesFound: candidates.length,
      enqueued: 0,
      exhausted,
      sampleJobKeys: candidates.map((candidate) => String(candidate.job_key)).slice(0, 5)
    };
  }

  const sampleJobKeys = (data ?? [])
    .map((row) => (isRecord(row) ? asString(row.job_key) : null))
    .filter((value): value is string => Boolean(value))
    .slice(0, 5);
  ...
  return {
    candidatesFound: candidates.length,
    enqueued: data?.length ?? candidates.length,
    exhausted,
    sampleJobKeys
  };
}
