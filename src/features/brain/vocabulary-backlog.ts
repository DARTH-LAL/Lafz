...
type EnqueueVocabularyBacklogBatchOptions = {
  limit?: number;
};

type VocabularyBacklogBatchResult = {
...
export function getVocabularyBacklogRefillCooldownMs() {
...
export function isVocabularyBacklogAutoRefillEnabled() {
...
function buildVocabularyAgentJobKey(spotifyTrackId: string, generatedAt: string) {
  return ["vocabulary_agent", spotifyTrackId, generatedAt].join("::");
}

function buildVocabularyPayload(draft: UnknownRecord, songNodeId: string, fallbackTrackId: string, fallbackGeneratedAt: string) {
...
export async function hasActiveVocabularyAgentJobs() {
  ...
  if (error) {
    console.error("[lafz-brain] could not count active vocabulary jobs.", error);
    return false;
  }
...
export async function enqueueVocabularyBacklogBatch(
  options: EnqueueVocabularyBacklogBatchOptions = {}
): Promise<VocabularyBacklogBatchResult> {
  const supabase = getSupabaseServerClient();
  ...
  const [draftRows, worldRows, existingJobRows] = await Promise.all([
    ...
    console.error("[lafz-brain] could not scan vocabulary backlog.", error);
    return [[], [], []] as const;
  });
  ...
  if (error) {
    console.error("[lafz-brain] could not enqueue vocabulary backlog batch.", error);
    return {
