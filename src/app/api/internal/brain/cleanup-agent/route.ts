...
export async function POST(request: NextRequest) {
  ...
  const workerId =
    ...
    process.env.LAFZ_CLEANUP_AGENT_WORKER_ID?.trim() ??
    `lafz-standalone-cleanup-worker-${process.pid}`;

  const processed = await runCleanupAgentUntilIdle({
    ignoreMode: true,
    workerId,
    reason: "remote",
    maxJobs
  ...
  return NextResponse.json({
    ...
    processed,
    status: getCleanupAgentProcessStatus()
  });
}
