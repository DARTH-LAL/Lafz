...
import {
  ...
  getPersonaAgentProcessStatus,
  kickPersonaAgentWorker,
  runPersonaAgentUntilIdle
} from "@/features/brain/persona-agent";
...
export async function POST(request: NextRequest) {
  ...
  const workerId =
    ...
    process.env.LAFZ_PERSONA_AGENT_WORKER_ID?.trim() ??
    `lafz-standalone-persona-worker-${process.pid}`;

  const processed = await runPersonaAgentUntilIdle({
    ignoreMode: true,
    workerId,
    reason: "remote",
    maxJobs
  ...
  ensurePersonaAgentWorkerStarted();
  kickPersonaAgentWorker("remote-followup");
  ...
  return NextResponse.json({
    ...
    processed,
    status: getPersonaAgentProcessStatus()
  });
}
