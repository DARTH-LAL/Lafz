...
import {
  ...
  getMotifAgentProcessStatus,
  kickMotifAgentWorker,
  runMotifAgentUntilIdle
} from "@/features/brain/motif-agent";
...
export async function POST(request: NextRequest) {
  ...
  const workerId =
    ...
    process.env.LAFZ_MOTIF_AGENT_WORKER_ID?.trim() ??
    `lafz-standalone-motif-worker-${process.pid}`;

  const processed = await runMotifAgentUntilIdle({
    ignoreMode: true,
    workerId,
    reason: "remote",
    maxJobs
  ...
  ensureMotifAgentWorkerStarted();
  kickMotifAgentWorker("remote-followup");
  ...
  return NextResponse.json({
    ...
    processed,
    status: getMotifAgentProcessStatus()
  });
}
