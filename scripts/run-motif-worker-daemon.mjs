...
function parseArgs(argv) {
  const options = {
    ...
    intervalMs: null,
    maxJobs: 3,
    workerId: process.env.LAFZ_MOTIF_AGENT_WORKER_ID?.trim() || `lafz-standalone-motif-worker-${process.pid}`
  };
...
async function runTick({ appUrl, secret, maxJobs, workerId }) {
  const response = await fetch(`${appUrl}/api/internal/brain/motif-agent`, {
    method: "POST",
    headers: {
...
async function main() {
  ...
  console.log("[motif-worker-daemon] starting", {
    appUrl,
    intervalMs,
  ...
  do {
    try {
      ...
      console.log("[motif-worker-daemon] tick", {
        processedCount: result?.processedCount ?? 0,
        workerId: result?.workerId ?? options.workerId,
        runtimeMode: result?.status?.runtimeMode ?? null
      });
    } catch (error) {
      console.error("[motif-worker-daemon] tick failed.", error instanceof Error ? error.message : String(error));
    }
...
main().catch((error) => {
  console.error("[motif-worker-daemon] fatal error.", error instanceof Error ? error.message : String(error));
  process.exitCode = 1;
});
