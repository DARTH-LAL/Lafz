...
function parseArgs(argv) {
  const options = {
    ...
    intervalMs: null,
    maxJobs: 3,
    workerId: process.env.LAFZ_ENTITY_AGENT_WORKER_ID?.trim() || `lafz-standalone-entity-worker-${process.pid}`
  };
...
async function runTick({ appUrl, secret, maxJobs, workerId }) {
  const response = await fetch(`${appUrl}/api/internal/brain/entity-agent`, {
    method: "POST",
    headers: {
...
async function main() {
  ...
  console.log("[entity-worker-daemon] starting", {
    appUrl,
    intervalMs,
  ...
  do {
    try {
      ...
      console.log("[entity-worker-daemon] tick", {
      ...
      console.error(
        "[entity-worker-daemon] tick failed.",
...
main().catch((error) => {
  console.error("[entity-worker-daemon] fatal error.", error instanceof Error ? error.message : String(error));
  process.exitCode = 1;
});
