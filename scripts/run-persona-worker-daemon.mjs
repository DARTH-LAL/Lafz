...
function parseArgs(argv) {
  const options = {
    ...
    intervalMs: null,
    maxJobs: 3,
    workerId: process.env.LAFZ_PERSONA_AGENT_WORKER_ID?.trim() || `lafz-standalone-persona-worker-${process.pid}`
  };
...
async function runTick({ appUrl, secret, maxJobs, workerId }) {
  const response = await fetch(`${appUrl}/api/internal/brain/persona-agent`, {
    method: "POST",
    headers: {
      "content-type": "application/json",
      authorization: `Bearer ${secret}`
    },
    body: JSON.stringify({ maxJobs, workerId })
  });
...
async function main() {
  ...
  console.log("[persona-worker-daemon] starting", {
    appUrl,
    intervalMs,
  ...
  do {
    try {
      ...
      console.log("[persona-worker-daemon] tick", {
        processedCount: result?.processedCount ?? 0,
        workerId: result?.workerId ?? options.workerId,
        runtimeMode: result?.status?.runtimeMode ?? null
      });
    } catch (error) {
      console.error("[persona-worker-daemon] tick failed.", error instanceof Error ? error.message : String(error));
    }
...
main().catch((error) => {
  console.error("[persona-worker-daemon] fatal error.", error instanceof Error ? error.message : String(error));
  process.exitCode = 1;
});
