...
function parseArgs(argv) {
  const options = {
    ...
    intervalMs: null,
    maxJobs: 3,
    workerId: process.env.LAFZ_AGENT_WORKER_ID?.trim() || `lafz-standalone-worker-${process.pid}`
  };
...
async function runTick({ appUrl, secret, maxJobs, workerId }) {
  const response = await fetch(`${appUrl}/api/internal/brain/vocabulary-agent`, {
    method: "POST",
    headers: {
...
async function main() {
  ...
  console.log("[vocabulary-worker-daemon] starting", {
    appUrl,
    intervalMs,
  ...
  do {
    try {
      ...
      console.log("[vocabulary-worker-daemon] tick", {
      ...
      console.error(
        "[vocabulary-worker-daemon] tick failed.",
...
main().catch((error) => {
  console.error("[vocabulary-worker-daemon] fatal error.", error instanceof Error ? error.message : String(error));
  process.exitCode = 1;
});
